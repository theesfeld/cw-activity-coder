...
(defun cw-activity-coder--load-activity-codes ()
  "Load activity codes from file or initialize with base if missing, ensuring ASCII."
  (unless cw-activity-coder-activity-codes
    ...
    (if (file-exists-p cw-activity-coder-activity-codes-file)
        (with-temp-buffer
          (insert-file-contents cw-activity-coder-activity-codes-file)
          (set-buffer-multibyte nil) ; Treat as unibyte
          (goto-char (point-min))
