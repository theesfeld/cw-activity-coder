                   ...
                   ((type . "object")
                    (properties
                     ...
                     ((ref . ((type . "string")))
                      (cw_at . ((type . "string")))))
                    (required . ["ref" "cw_at"]))))))))))))
    (while (>= cw-activity-coder--active-requests
     ...
     (cl-function
      (lambda (&key data &allow-other-keys)
        (let* ((content
                ...
                (mapcar
                 (lambda (item)
                   (let ((ht
                     ...
                     (list
                      (cons "ref" (gethash "ref" ht))
                      (cons "cw_at" (gethash "cw_at" ht)))))
                 (if (vectorp raw-result)
                     (append raw-result nil)
                   raw-result)))
          ...
          (message "DEBUG: Processed batch with %d items"
                   (length result))
...
(defun cw-activity-coder--process-batch
    (start-line end-line callback total-batches processed-batches)
      ...
      (let ((start-time (float-time)))
        (cw-activity-coder--api-request
         ...
         (lambda (batch-result error)
           ...
           (message "DEBUG: Batch from %d to %d completed"
                    start-line
                    end-line)
           (setq cw-activity-coder--progress
                 (format "CW: %d/%d batches"
                         (1+ processed-batches)
                         total-batches))
          ...
          (dotimes (i batches)
            (let ((start-line
              ...
              (cw-activity-coder--process-batch
               start-line end-line
               (lambda (batch-result
                        error total-batches new-processed)
                 (if error
                     (push error errors)
                 ...
                 (when (= new-processed total-batches)
                   (if errors
                       (progn
                         (setq cw-activity-coder--progress
                               (format "CW: Failed with %d errors"
                                       (length errors)))
                         (force-mode-line-update)
                         (error
                          "Processing failed: %s"
...
(add-to-list 'mode-line-misc-info '(:eval cw-activity-coder--progress)
             t)
